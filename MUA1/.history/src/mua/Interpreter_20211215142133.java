...
public class Interpreter {
    ...
    public Word parse(String cmd) {
        ...
        //列表存储，内容都当作String存储
            ...
            */
            ...
            //返回当前作用域中的变量
            if(ScopeFunc.get(Scope.size() - 1).containsKey(name)) {
                return ScopeFunc.get(Scope.size() - 1).get(name);
            }
            else {
    ...
    public Boolean IsDefinedOperate(String func) {
        ...
        if(ScopeFunc.get(Scope.size() - 1).containsKey(func)) {
            return true;
        }
        else {
            for(int i = 0; i < LScope.size(); i++) {
    ...
    public Word getDefinedFunc(String func) {
        ...
        if(ScopeFunc.get(Scope.size() - 1).containsKey(func)) {
            return ScopeFunc.get(Scope.size() - 1).get(func);
        }
        else {
            for(int i = 0; i < LScope.size(); i++) {
                if(ScopeFunc.get(LScope.get(i)).containsKey(func)) {
