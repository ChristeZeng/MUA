...
public class Interpreter {
    ...
    private HashMap<int, Word> OpMem = new HashMap<>();
    ...
    public Word parse(String cmd) {
        ...
        //如果是列表
            ...
            if(Scope != 0 && OpMem.get(Scope).LoaclVar.containsKey(name)) {
            ...
            Word tmp = getInput();
            if(Scope != 0 && OpMem.get(Scope).LoaclVar.containsKey(tmp.getString())) {
            ...
            if(DMem.containsKey(tmp.getString())) {
                ...
                DMem.replace(tmp.getString(), OpMem.get(Scope).LoaclVar.get(tmp.getString()));
            }
            else
                DMem.put(tmp.getString(), OpMem.get(Scope).LoaclVar.get(tmp.getString()));
            return OpMem.get(Scope).LoaclVar.get(tmp.getString());
        }
