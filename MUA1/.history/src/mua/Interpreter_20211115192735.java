...
public class Interpreter {
    ...
    public Word parse(String cmd) {
        ...
        //列表存储，内容都当作String存储
            ...
            //返回当前作用域中的变量
            if(CurrentFunc.length() != 0 && ScopeFunc.get(ScopeFunc.size() - 1).get(CurrentFunc).LoaclVar.containsKey(name)) {
                return ScopeFunc.get(ScopeFunc.size() - 1).get(CurrentFunc).LoaclVar.get(name);
            }
            //如果当前作用域中没有，则返回全局变量
            else {
            ...
            if(ScopeFunc.size() > 1) {
                ScopeFunc.get(ScopeFunc.size() - 1).get(CurrentFunc).LoaclVar.put(name, tmp);
            }
            else {
            ...
            Word tmp = getInput();
            if(ScopeFunc.size() > 1) {
                ScopeFunc.get(ScopeFunc.size() - 1).get(CurrentFunc).LoaclVar.remove(tmp.getString());
            }
            else {
            ...
            Word tmp = new Word();
            if(ScopeFunc.size() > 1) {
                if(ScopeFunc.get(ScopeFunc.size() - 1).get(CurrentFunc).LoaclVar.containsKey(name)) {
                    tmp.assign("true", 2);
                }
                else {
                    tmp.assign("false", 2);
            ...
            if(ScopeFunc.get(0).containsKey(tmp.getString())) {
                ScopeFunc.get(0).replace(tmp.getString(), ScopeFunc.get(ScopeFunc.size() - 1).get(CurrentFunc).LoaclVar.get(tmp.getString()));
            }
            else {
                ScopeFunc.get(0).put(tmp.getString(), ScopeFunc.get(ScopeFunc.size() - 1).get(CurrentFunc).LoaclVar.get(tmp.getString()));
            }
            ...
            //     System.out.println(Func.ArgList.get(i));
            // }
            //获取执行命令表
            int LeftBracket = 1;
            int RightBracket = 0;
            ...
            //System.out.println(Func.getOpString());
            //参数赋值
            for(int i = 0; i < Func.ArgList.size(); i++) {
                Word argment = getInput();
                Func.LoaclVar.put(Func.ArgList.get(i), argment);
            }
            //print the Func.LoaclVar
            // for(Map.Entry<String, Word> entry : Func.LoaclVar.entrySet()) {
